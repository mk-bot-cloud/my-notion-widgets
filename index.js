...
const DB_ACADEMIC_EVENT_ID = process.env.DB_ACADEMIC_EVENT_ID; 
...
async function main() {
  try {
    ...
    console.log("\n=== 3. 学術大会・研修会情報収集 ===");
    if (DB_ACADEMIC_EVENT_ID) {
      await fetchAllConferences();
      await fetchNptaWorkshops();
    }
    console.log("\n=== 4. PubMed要約 ===");
    ...
    for (const row of rows) {
      ...
      if (cells.length >= 5) {
        ...
        if (link && link.startsWith('http')) {
          const exists = await notion.databases.query({ database_id: DB_ACADEMIC_EVENT_ID, filter: { property: "URL", url: { equals: link } } });
          if (exists.results.length === 0) {
            await notion.pages.create({
              parent: { database_id: DB_ACADEMIC_EVENT_ID },
              properties: {
            ...
            console.log(`✅ 学会登録: ${conferenceName}`);
          }
        }
      }
    }
  } catch (e) { console.error("学会エラー:", e.message); }
}

async function fetchNptaWorkshops() {
  try {
    const res = await axios.get("http://www.npta.or.jp/wp/", { headers: { "User-Agent": "Mozilla/5.0" } });
    const $ = cheerio.load(res.data);
    const items = $('.post, article, .entry-title').get(); 
    for (const item of items.slice(0, 10)) {
      const anchor = $(item).find('a').first().length ? $(item).find('a').first() : $(item).is('a') ? $(item) : null;
      if (!anchor) continue;
      const title = anchor.text().trim();
      const link = anchor.attr('href');
      if (link && (link.includes('workshop') || title.includes('研修会') || title.includes('講習会'))) {
        const exists = await notion.databases.query({ database_id: DB_ACADEMIC_EVENT_ID, filter: { property: "URL", url: { equals: link } } });
        if (exists.results.length === 0) {
          await notion.pages.create({
            parent: { database_id: DB_ACADEMIC_EVENT_ID },
            properties: {
              '大会名称': { title: [{ text: { content: title } }] },
              'URL': { url: link },
              '種別': { select: { name: "研修会" } }
            }
          });
          console.log(`✅ 研修会登録: ${title}`);
        }
      }
    }
  } catch (e) { console.error("研修会エラー:", e.message); }
}
...
async function generateQuestionsFromSummaries() {
  try {
    ...
    const validPages = res.results.filter(page => (page.properties['要約']?.rich_text[0]?.plain_text || "").length > 10);
    if (validPages.length === 0) return;
    const materials = validPages.map(page => `【${page.properties['タイトル和訳']?.rich_text[0]?.plain_text || "無題"}】: ${page.properties['要約']?.rich_text[0]?.plain_text || ""}`).join("\n\n");
    const prompt = `高度な専門性を持つリハビリテーション研究者・教員として、以下から「本質的な問い」を3つ提案してください。JSON形式: { "actions": [ { "q": "疑問文" } ] }\n\n${materials}`;
    ...
    for (const item of aiData.actions) {
      ...
      if (exists.results.length === 0) {
        await notion.pages.create({ parent: { database_id: DB_ACTION_ID }, properties: { '問い': { title: [{ text: { content: item.q } }] } } });
      }
    }
...
async function fetchNewsDaily() {
  const sources = [{ name: "ICT教育ニュース", url: "https://ict-enews.net/feed/" }, { name: "ITmedia AI+", url: "https://rss.itmedia.co.jp/rss/2.0/aiplus.xml" }];
