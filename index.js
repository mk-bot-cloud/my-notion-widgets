  ...
  for (const source of sources) {
    ...
    try {
      ...
      for (const item of feed.items) {
        ...
        const cleanTitle = rawTitle.replace(/[\[【].*?[\]】]/g, '').replace(/^ITmedia\s*[:：]\s*/g, '').trim();
...
async function extractContent(url) {
  try {
    ...
    const bodyMatch = html.match(/<div\s+id=["']cmsBody["']>([\s\S]*?)<\/div>/) || html.match(/<div\s+class=["']entry-content["']>([\s\S]*?)<\/div>/);
...
async function createNotionPage(title, link, imageUrl, bodyText) {
  ...
  const chunks = bodyText.match(/[\s\S]{1,2000}/g) || [bodyText];
  chunks.forEach(chunk => children.push({ object: "block", type: "paragraph", paragraph: { rich_text: [{ text: { content: chunk } }] } }));
  ...
  await notion.pages.create({
    ...
    properties: { '名前': { title: [{ text: { content: title } }] }, 'URL': { url: link } },
    children: children
  });
}
run();
